...
void SteadySmallTraffic::Run() {
    ...
    const PhaseConfig& sendPhase = _config.FindPhase(PhaseType::SteadySend);
    ...
    _logger->Info("[3/4] steady_send ({}s, interval={}ms, size={}B)",
                  sendPhase.steady_send.duration_sec, sendPhase.steady_send.send_interval_ms,
                  sendPhase.steady_send.message_size_bytes);
    SteadySend(_sessions, sendPhase.steady_send.duration_sec, sendPhase.steady_send.send_interval_ms,
               sendPhase.steady_send.message_size_bytes, _logger);
