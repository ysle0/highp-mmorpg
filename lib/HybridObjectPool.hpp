	...
	struct LocalStorage {
		...
		~LocalStorage() {
			...
			for (int i = 0; i < pool.size(); i++) {
				T* lend = pool[i];
				HybridObjectPool::_globalPool.emplace_back(lend);
			}
			pool.clear();
		}
	};
...
public:
	...
	static T* Get() {
		...
		// 없으면 chunk 하나 때옴
		{
			...
			if (size > 0) {
				const int fetchCount = size > HybridObjectPoolConfig::ChunkSize ?
					HybridObjectPoolConfig::ChunkSize : size;
	...
	static void Return(T* item) {
		...
		if (local.pool.size() >= HybridObjectPoolConfig::LocalMaxCapacity) {
			...
			const int move = HybridObjectPoolConfig::ChunkSize;
