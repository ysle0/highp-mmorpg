...
| 클래스 | 역할 | 레이어 |
|--------|------|--------|
| **IoCompletionPort** | IOCP 핸들 관리 및 Worker 스레드 풀 운영 | Infrastructure |
| **Acceptor** | AcceptEx 기반 비동기 연결 수락 | Infrastructure |
| **EchoServer** | 비즈니스 로직 (Echo 처리) 및 컴포넌트 조율 | Application |
| **Client** | 클라이언트 연결 상태 및 Overlapped 버퍼 관리 | Domain |
| **OverlappedExt** | 비동기 I/O 컨텍스트 (WSAOVERLAPPED 확장) | Infrastructure |
| **WindowsAsyncSocket** | Listen 소켓 생성/바인딩/리스닝 | Infrastructure |
...
- `AssociateSocket(SocketHandle, void*)` - 소켓을 IOCP에 연결
...
│    └─ WSAIoctl로 AcceptEx 함수 포인터 획득              │
│    └─ OverlappedExt 풀 사전 할당                        │
│                                                         │
│  PostAccept()                                           │
...
- `OnAcceptComplete(OverlappedExt*, DWORD)` - Accept 완료 처리
- `SetAcceptCallback(AcceptCallback)` - 연결 수락 콜백 등록

**AcceptEx 간접 호출:**
```cpp
// WSAIoctl로 함수 포인터 획득 (직접 호출 X)
...
// 함수 포인터로 간접 호출
_fnAcceptEx(_listenSocket, acceptSocket, buffer, ...);
```
...
**OverlappedExt 캐스팅 규칙:**
```cpp
// WSAOVERLAPPED가 첫 번째 멤버여야 함
struct OverlappedExt {
    WSAOVERLAPPED overlapped{};  // offset 0
    EIoType ioType;
    // ...
};

// 캐스팅 (양방향)
reinterpret_cast<LPOVERLAPPED>(overlappedExt)     // 전달 시
reinterpret_cast<OverlappedExt*>(lpOverlapped)    // 수신 시
```
