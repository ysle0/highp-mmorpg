...
namespace highp::network {
    ...
    public:
        ...
        /// <param name="handler">완료 이벤트 처리 콜백 (선택적). RAII 원칙에 따라
        /// 생성 시 설정 권장.</param>
        explicit IocpIoMultiplexer(std::shared_ptr<log::Logger> logger,
        ...
        /// </summary>
        /// <param name="workerThreadCount">생성할 Worker 스레드 수. 일반적으로 CPU
        /// 코어 수 권장.</param> <returns>성공 시 Ok, 실패 시 에러 코드</returns>
        /// <remarks>
        ...
        /// <param name="completionKey">완료 이벤트 시 전달받을 키. 일반적으로 Client
        ...
        /// <summary>IOCP 커널 핸들을 반환한다. IocpAcceptor::Initialize()에
        /// 전달용.</summary> <returns>IOCP 핸들</returns>
    ...
    private:
        ...
        /// <summary>Worker 스레드 목록. std::jthread로 자동 join 및 취소
        /// 지원.</summary>
